...
let text = "Helping your home help your planet";
        ...
        <motion.h1 variants={headingVariants}  initial="initial" animate="animate" className="overflow-hidden max-w-[1280x] text-center mx-auto" >
            {text.split(" ").map((word, i ) => (
                    <span key={i}className={classNames(word, "inline-block overflow-hidden relative")}>
                        {word == "home" && <motion.div key={word + i} initial={{ right: "100%" }} animate={{ right: "8%" }} transition={{ delay: 1.6}} className="absolute left-0 h-full bg-green-600" /> }
                        {word == "planet" && <motion.div key={word + i} initial={{ right: "100%" }} animate={{ right: "5%" }} transition={{ delay: 1.8}} className="absolute left-0 h-full bg-purple-600" /> }
