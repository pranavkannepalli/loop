...
export default function ROIGraph({
    ...
    const stuffClose =
        Math.abs(max[1] - target[1]) / verticalRange < 0.05 ? true : false;
    const polygon = getPolygon(points);
    return (
        <div
            ref={ref}
            style={{ aspectRatio: "5/3" }}
            className="flex-1 flex flex-col overflow-hidden bg-white-100 border border-white-300 rounded-[20px]"
        >
            ...
            <div className="points relative flex-1">
                ...
                <motion.div
                    id="graph"
                    className={classNames(
                        "absolute w-full h-full bg-gradient-to-b from",
                        barGradient
                    )}
                    initial={{ scaleY: 0 }}
                    animate={
                ...
                <div
                    ...
                    style={{
                        bottom:
                            ((target[1] - min[1]) * 100) / verticalRange + "%",
                        left: 0,
                    }}
                >
                    <div className="h-[2px] bg-white-600 flex-1" />
                    <p className="caption px-[20px]">
                        ${target[1].toLocaleString()}
                    </p>
                ...
                <div
                    ...
                    style={{
                        left:
                            ((target[0] - start[0]) * 100) / horizontalRange +
                            "%",
                        bottom: 0,
                        height:
                            ((target[1] - min[1]) * 100) / verticalRange + "%",
