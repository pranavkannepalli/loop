...
export default function Solution({ params }) {
    ...
    const { userData, addWatchlist, inWatchlist, addItem, removeWatchlist } =
    ...
    const getButton = () => {
        if (loggedIn) {
            if (isIn)
                return (
                    <Button
                        prefix={<Icon name="remove" size={20} />}
                        onClick={() => removeWatchlist(solution)}
                    >
            ...
            <main>
                <section className="layout box-border md:h-[100vh] items-center !mt-[100px] md:!mt-0 w-full">
                    <div className="flex flex-col-reverse gap-[20px] md:flex-row items-center justify-center w-full md:h-[100vh]">
                        <div className="flex-1 w-full text-left flex flex-col items-stretch md:items-start gap-[20px]">
                            ...
                            >
                                <Icon
                                    name={filterData[solution.type].icon}
                                    size={20}
                                    className={
                                        filterData[solution.type].iconColor
                                    }
                                />
                                <h6 className={"text-white-600"}>
                                    {solution.type}
                                </h6>
                            </div>
                            <div>
                            ...
                            <div className="flex flex-row gap-[20px] flex-wrap">
                                {solution.stats.map((val, ind) => (
                                    <div
                                        key={ind}
                                        className="flex-1 min-w-[150px] py-[20px] px-[20px] rounded-[10px] border border-white-300 flex flex-col gap-[5px]"
                                    >
                ...
                <section className="layout box-border items-center !mt-[50px] w-full">
                    <div className="flex flex-col gap-[20px] md:flex-row justify-center w-full">
                        <div className="flex-1 w-full text-left flex flex-col md:items-start gap-[20px]">
                            ...
                            {(solution.steps ?? defaultSteps).map(
                                (item, ind) => (
                                    <Accordion
                                        number={ind + 1}
                                        key={ind}
                        ...
                        <div className="flex-1 text-left">
                            ...
                            {solution.roiGraph[
                                solution.roiGraph.length - 1
                            ][1] != 0 && (
                                <ROIGraph
                                    points={solution.roiGraph}
                                    cost={solution.price}
                                />
                            )}
                        </div>
                ...
                <section className="layout box-border !mt-[30px] w-full">
                    ...
                    <div className="flex flex-col gap-[20px] md:flex-row justify-center w-full mt-[30px]">
                        <div className="flex-1 w-full text-left flex flex-col md:items-start gap-[20px]">
                            <div className="caption text-white-400">
                                Utility
                            </div>
                            {(solution.utilityRebates ?? []).map(
                                (item, ind) => (
                                    <Rebate
                                        dollars={item.price}
                                        label={item.label}
